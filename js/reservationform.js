...
function showResponseContact(responseText, statusText)  { 
	if (statusText == 'success') {
		jQuery('#reservation-holder').html('<h5>Message sent</h5>'); 
...
function showRequestContact(formData, jqForm, options2) { 
	...
	if (!form.name.value) { 
		...
		jQuery('#output-contact').html('<div class="output2">Please fill the Date field!</div>'); 
		return false; 
	}
	else if (!form.seats.value) {
		jQuery('#output-contact').html('<div class="output2">Please fill the Seats field!</div>'); 
		return false; 
	}
